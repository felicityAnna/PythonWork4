...
def create_mn(koef):
    list = [rnd() for i in range(koef + 1)]
    return list
...
def sq_mn(koef):
    if 'x^' in koef:
        i = koef.find('^')
        num = int(koef[i+1:])
    elif ('x' in koef) and ('^' not in koef):
        num = 1
    else:
        num = -1
    return num
...
koef1 = create_mn(k1)
koef2 = create_mn(k2)
write_file("task_4_2_1.txt", create_str(koef1))
write_file("task_4_2_2.txt", create_str(koef2))

# нахождение суммы многочлена

with open('task_4_2_1.txt', 'r') as data:
    st1 = data.readlines()
with open('task_4_2_2.txt', 'r') as data:
    st2 = data.readlines()
...
write_file("task_4_2_3.txt", create_str(lst_new))
with open('task_4_2_3.txt', 'r') as data:
    st3 = data.readlines()
